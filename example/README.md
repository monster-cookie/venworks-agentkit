...
6. When using Proton Pass, keep the bootstrap PAT in the protected `PROTON_PASS_PERSONAL_ACCESS_TOKEN` environment variable supplied by the user's local setup; never put it in a Proton Pass item, policy, file, argument, or `pass://` reference because that would create a circular dependency. Keep Expected identity focused on the downstream consumer account or principal. Use `pass-cli info` to verify a healthy dedicated session, and compare a token name only when the user explicitly supplied one; an undocumented token name is not a reason to refuse PAT login.
