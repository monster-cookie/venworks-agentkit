...
When Manager is Proton Pass via Proton Pass CLI (`pass-cli`), the expected Proton agent name must be available as nonsecret Authentication metadata, such as `expected Proton agent name=replace-with-proton-agent-name`, or from explicit user-provided setup input. Keep this manager identity separate from Expected identity, which names the downstream service account or principal. Before credential access, match the PAT name reported by `pass-cli info` to the supplied effective value; never learn the expected name from the observed session. If the effective input is missing, unfinished, or conflicting, defer only the dependent credential access; a healthy already-verified consuming session remains usable.
