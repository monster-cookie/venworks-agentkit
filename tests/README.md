...
For a Proton Pass decision exercise, use synthetic command results without live secrets: a correct consuming-tool login should need no credential read; a new agent session must use its own directory before login; a confirmed expired session may recover through the supplied PAT; a permission or network failure must not trigger logout; duplicate item titles must not select the first item; and an uncertain mutation or `run` outcome must be inspected before retry. Verify that missing or conflicting expected Proton agent names defer only Proton-dependent access, that the manager name comes from `Authentication` metadata or explicit user setup rather than the observed session or downstream account, and that access reasons describe the task. These are manual behavior checks, not outcomes established by the installer suite.
