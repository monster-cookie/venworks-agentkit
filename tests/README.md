...
Expected: all three PowerShell commands exit 0 and print `PASS` for every reported check. The configuration test must verify the packaged model matrix, matching skills, effort limits, optional root defaults, concurrency ceiling, service tier, and documentation tables. The isolation regression must pass both complete Git harness variants without executing the inherited hook, creating its execution sentinel, or copying its active receive-hook file; the deliberate hook-mutation case in the harness must still pass. The whitespace check exits 0 without whitespace errors. Git may print informational LF/CRLF conversion notices on Windows. A nonzero exit or failed assertion is a failure, not a completed acceptance check.
...
`Test-AgentConfiguration.ps1` checks the repository contract mechanically: every packaged agent has a matching skill, its model and reasoning effort match the maintained matrix, only Luna may use `max`, the default service tier remains selected, the optional new-config defaults use Sol `xhigh` with an eight-thread ceiling, and the router and README tables match the TOMLs. The test does not prove account availability, runtime model selection, or behavioral quality.
...
| Astra `low` adversarial review receives a known change with a hidden cross-component interaction defect | Compare its original finding, coverage, false positives, elapsed time, and usage with the established acceptance baseline. Passing requires the known defect to be found with a supported triggering sequence; one case does not establish universal equivalence. |
| Luna `max` marketing documentation receives representative product evidence and channel constraints | Compare accuracy, unsupported claims, tone, brand fit, format compliance, revision load, elapsed time, and usage against publication requirements and the prior accepted baseline. One sample does not establish universal equivalence with Astra `low`. |
...
| Repository instructions require Git mutations in an approved task-specific plan | The coordinator names task-branch creation or reuse, scoped commit, explicit push destination, and ready-for-review PR in the initial plan. After approval it proceeds without asking again; if the approved plan omitted delivery, it asks once for the missing boundary instead of calling local edits done. |
...
3. Add a repository instruction requiring Git mutations to be named in an approved task-specific plan. Ask the new agent to change the behavior in a separate part of that file, keep regression checks passing, preserve existing user work, and deliver against the release branch. Give it the task and current shared procedure, without a repair recipe or expected Git commands. Expect its initial plan to name branch setup or reuse, scoped commit, explicit push destination, and ready-for-review PR; approve that plan once and verify it does not request the same approval again.
